<?php /*a:1:{s:78:"E:\Code\Git-tp6\ThinkPHP6-Shopping_Project\tp\app\admin\view\specs\dialog.html";i:1610345969;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="layui-inline more-right">
    <div class="spec-title">选择规格值[可多选]
        <input type="text" id="add-spec-tags" style="padding-left: 5px; width: 252px;margin-left: 10px;
        display:inline-block;
" placeholder="新增规格名、按回车键确定添加" autocomplete="off" class="layui-input">
    </div>
    <div class="layui-input-inline">
    ...
    layui.use(['form'], function () {
        ...
        function queryClassif() { // 请求分类 后端接口
            ...
            moke.forEach(function (item, index) {
                html += `<li class="child-ele" data-id="${item.id}" data-specs_id="${item.specs_id}"  data-index="${index}">${item.name}</li>`
            })
        ...
        $('body').on('click', '.child-ele', function () {
            ...
            layObj.get(url, function (res) {
                ...
                projectArr.forEach(function (item) {
                    html += '<button type="button" data-specs-value-id="' + item.id + '" ' + 'class="layui-btn layui-btn-sm layui-btn-normal" style="margin-top: 12px"> ' +
                        '<b>' + item.name + '</b><i class="layui-icon del-child-tags" data-id="' + item.id + '" ' +
                        'style="padding-left: 12px;"></i></button>'
                })
        ...
        $('html').on('click', '.del-child-tags', function () {
            ...
            layObj.box(`是否删除该规格`, () => {
                let url = '<?php echo url("admin/SpecsValue/status"); ?>?id=' + id + "&status=99"
                layObj.get(url, (res) => {
                    if (res.status == 1) {
                        layer.msg("删除成功");
